...
import { LayoutDashboard, LogOut, Loader2, DollarSign, FileText, TrendingUp, Target, Clock, PieChart, Plus, BarChart2 } from 'lucide-react';
  ...
  return (
    <div className="card card-wide">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h2 style={{ margin: 0, fontSize: '2rem', fontWeight: '800', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
        ...
        <button onClick={logout} className="danger" style={{ width: 'auto', padding: '0.6rem 1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
      ...
      {loading && (
        <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--fg-muted)' }}>
          <Loader2 size={36} className="spin" style={{ marginBottom: '1rem', display: 'block', margin: '0 auto 1rem' }} />
          Loading dashboard...
        </div>
      )}
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.25rem', marginBottom: '2.5rem' }}>
            <div className="stat-card" style={{ background: 'linear-gradient(135deg, rgba(59, 130, 246, 0.15) 0%, rgba(37, 99, 235, 0.15) 100%)' }}>
              <div className="stat-label" style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}><DollarSign size={14} /> This Month Total</div>
            ...
            <div className="stat-card" style={{ background: 'linear-gradient(135deg, rgba(16, 185, 129, 0.15) 0%, rgba(5, 150, 105, 0.15) 100%)' }}>
              <div className="stat-label" style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}><FileText size={14} /> Transactions</div>
              <div className="stat-value">
                {summary?.count || 0}
              </div>
            </div>
            <div className="stat-card" style={{ background: 'linear-gradient(135deg, rgba(245, 158, 11, 0.15) 0%, rgba(217, 119, 6, 0.15) 100%)' }}>
              <div className="stat-label" style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}><TrendingUp size={14} /> Avg Transaction</div>
              <div className="stat-value">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: '2rem' }}>
            ...
            <section style={{ 
              ...
              <h3 style={{ marginTop: 0, marginBottom: '1.5rem', fontSize: '1.25rem', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <Target size={20} /> Spending by Category
              </h3>
          ...
          <div style={{ marginTop: '2.5rem', paddingTop: '2rem', borderTop: '1px solid var(--glass-border)' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
              <button onClick={() => navigate('/expenses')} style={{ flex: '1', minWidth: '200px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                <Plus size={18} /> Add Expense
              </button>
              <button onClick={() => navigate('/reports')} className="success" style={{ flex: '1', minWidth: '200px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
